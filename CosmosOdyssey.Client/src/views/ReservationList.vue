...
<template>
  <div class="flex flex-col items-center p-4 lg:p-8">
    <!-- Search Box Section -->
    <div class="w-full sm:w-2/3 md:w-1/2 mb-6">
      <input
          v-model="searchQuery"
          type="text"
          placeholder="Search by last name"
          class="w-full p-4 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-300"
      />
    </div>

    <div v-if="reservations.length" class="relative text-center my-8 sm:my-12 px-6 sm:px-10 lg:px-14 w-full">
      <h2 class="text-2xl sm:text-3xl font-bold text-indigo-600 mb-6">Reservations</h2>

      <!-- Reservations displayed in a single line -->
    ...
    <div class="w-full sm:w-2/3 md:w-1/2 mt-6 sm:mt-12">
      <Button
          ...
          icon="pi pi-search"
          iconPos="left"
          class="w-full bg-indigo-600 text-white text-lg font-semibold py-4 rounded-lg shadow-md hover:bg-indigo-700 hover:scale-105 transform transition-all ease-in-out duration-300"
      />
    </div>
  </div>
</template>
